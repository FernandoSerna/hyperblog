...
Begin VB.Form frmpaqueteria_multipack 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Impresión de guias de multipack"
   ClientHeight    =   5820
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6780
   ...
   Begin VB.CommandButton cmd_nuevo 
      ...
      Height          =   315
      Left            =   120
      Picture         =   "frmpaqueteria_multipack.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   0
   ...
   Begin VB.CommandButton cmd_imprimir 
      ...
      Height          =   315
      Left            =   450
      Picture         =   "frmpaqueteria_multipack.frx":0102
      Style           =   1  'Graphical
      TabIndex        =   1
   ...
   Begin VB.CommandButton cmd_salir 
      ...
      Height          =   315
      Left            =   6315
      Picture         =   "frmpaqueteria_multipack.frx":0204
      Style           =   1  'Graphical
      TabIndex        =   2
      ToolTipText     =   "Salir"
      Top             =   15
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.PictureBox Picture1 
         ...
         Height          =   570
         Left            =   3750
         Picture         =   "frmpaqueteria_multipack.frx":083E
         ScaleHeight     =   570
         ScaleWidth      =   2730
         TabIndex        =   33
         Top             =   4545
   ...
   End
End
Attribute VB_Name = "frmpaqueteria_multipack"
   ...
   If Me.txt_cliente <> "" Then
      ...
      If var_si = 6 Then
         'If IsNumeric(Me.txt_orden) Then
            ...
            Print #1, ""
            Print #1, ""
            Print #1, "" 'Spc(112); Me.txt_paquetes
            Print #1, ""
            Print #1, ""
            Print #1, ""
            Print #1, Spc(80); "BLANCOS"
            Print #1, "" 'Spc(80); txt_valor
            If rs.State = 1 Then
               rs.Close
            End If
            Print #1, ""
            ...
            Print #1, ""
            Print #1, ""
            Print #1, Me.txt_representante
            var_colonia = Mid(Me.txt_colonia, 1, 22)
            Print #1, Spc(10); Me.txt_direccion
            For var_j = Len(Trim(var_colonia)) To 35
                var_colonia = var_colonia + " "
            Next var_j
            var_colonia = var_colonia + Trim(Mid(Me.txt_ciudad, 1, 15))
            Print #1, Spc(5); var_colonia
          
            var_estado = Mid(Me.txt_estado, 1, 15)
            For var_j = Len(Trim(var_estado)) To 35
                var_estado = var_estado + " "
            Next var_j
            var_estado = var_estado + Trim(Mid(Me.txt_municipio, 1, 15))
            Print #1, Spc(5); var_estado
            var_pais = Mid(Me.txt_pais, 1, 30)
            For var_j = Len(Trim(var_pais)) To 35
                var_pais = var_pais + " "
            Next var_j
            var_pais = var_pais + Trim(Me.txt_cp)
            Print #1, Spc(5); var_pais
            Print #1, ""
            Print #1, ""
            var_observacion_1 = ""
            var_observacion_2 = ""
            Me.txt_observacion = Mid(Me.txt_observacion, 1, 100)
            VAR_ZZ = Len(Me.txt_observacion)
            For var_n = 1 To VAR_ZZ
                If var_n <= 40 Then
                   var_observacion_1 = var_observacion_1 + Mid(Me.txt_observacion, var_n, 1)
                Else
                   var_observacion_2 = var_observacion_2 + Mid(Me.txt_observacion, var_n, 1)
                End If
            Next var_n
            'Print #1, Spc(15); "" 'var_observacion_1
            'Print #1, "" 'var_observacion_2
            'Print #1, ""
            Print #1, Spc(15); "FECHA:  " + CStr(Date)
            Print #1, ""
            Print #1, ""
            Print #1, ""
            Print #1, Spc(18); Me.txt_contable
            Print #1, ""
            Print #1, ""
            Print #1, ""
            Print #1, ""
      ...
      End If
   Else
      MsgBox "No se a seleccionado un cliente", vbOKOnly
   End If
End Sub
                     ...
                     If Not rsaux.EOF Then
                        ...
                        While Not rsaux.EOF
                              ...
                              Else
                                 var_cadena = var_cadena + ", " + CStr(IIf(IsNull(NUMERO_CAJAS), "", rsaux!NUMERO_CAJAS)) + " " + IIf(IsNull(rsaux!VCHA_CAJ_NOMBRE), "", rsaux!VCHA_CAJ_NOMBRE)
                              End If
                              VAR_NUMERO_CAJAS = VAR_NUMERO_CAJAS + IIf(IsNull(NUMERO_CAJAS), 0, rsaux!NUMERO_CAJAS)
                              rsaux.MoveNext
                        Wend
