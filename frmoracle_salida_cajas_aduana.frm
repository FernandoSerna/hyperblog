...
Private Sub cmd_cerrar_embarque_Click()
   If rsaux.State = 1 Then
      rsaux.Close
   End If
   rsaux.Open "SELECT PEDIDO FROM TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES WHERE EMBARQUE = " + Me.txt_embarque + " union all SELECT PEDIDO FROM TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES_VAD WHERE EMBARQUE = " + Me.txt_embarque, cnn, adOpenDynamic, adLockOptimistic
   var_Cadena_pedidos = ""
   Dim bln_pedidos_vad As Integer
   bln_pedidos_vad = 0
   While Not rsaux.EOF
         ...
         End If
         If Not IsNumeric(rsaux!pedido) And bln_pedidos_vad = 0 Then
            bln_pedidos_vad = 1
         End If
         rsaux.MoveNext
   Wend
   rsaux.Close
   If bln_pedidos_vad = 0 Then
   ...
   Else
    Call cmd_cerrar_embarque_nuevo_metodo_Click
    Exit Sub
   End If
   
   If rsaux.EOF Then
      rs.Open "select * from XXVIA_tB_ENCABEZADO_EMBARQUES  where embarque = " + Me.txt_embarque + " and (CHAR_EMB_ESTATUS = 'E' or CHAR_EMB_ESTATUS is null) ", cnnoracle_4, adOpenDynamic, adLockOptimistic
      If Not rs.EOF Then
...
Private Sub cmd_cerrar_embarque_nuevo_metodo_Click()
   ...
   'rs.Open "update TB_ORACLE_TIEMPO_EMBARQUE_ADUANAS_vad set HORA_FIN = GETDATE() where embarque = " + Me.txt_embarque, cnn, adOpenDynamic, adLockOptimistic
   ...
   'rs.Open "UPDATE TB_ORACLE_EMBARQUES_ORDENES_vad SET estatus = 'I' WHERE inte_emb_EMBARQUE = " + Me.txt_embarque, cnn, adOpenDynamic, adLockOptimistic
...
Private Sub cmd_cerrar_pedido_Click()
   If Me.lv_cajas.ListItems.Count > 0 Then
      'rs.Open "select pedido from TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES where embarque in (select embarque  from TB_ORACLE_GRUPOS_EMBARQUES where grupo = (select top 1 grupo from TB_ORACLE_GRUPOS_EMBARQUES where embarque = '" + Me.txt_embarque + "')) and PEDIDO not in (select pedido from tb_oracle_pedidos_Cerrados_cn union all select pedido from tb_oracle_pedidos_Cerrados)"
      rs.Open "select pedido from TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES where embarque in (select embarque  from TB_ORACLE_GRUPOS_EMBARQUES where grupo = (select top 1 grupo from TB_ORACLE_GRUPOS_EMBARQUES where embarque = '" + Me.txt_embarque + "')) and PEDIDO not in (select pedido from tb_oracle_pedidos_Cerrados_cn union all select pedido from tb_oracle_pedidos_Cerrados) " & _
              "union all " & _
              "select pedido from TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES_VAD where embarque in (select embarque  from TB_ORACLE_GRUPOS_EMBARQUES where grupo = (select top 1 grupo from TB_ORACLE_GRUPOS_EMBARQUES where embarque = '" + Me.txt_embarque + "')) and PEDIDO not in (select pedido from tb_oracle_pedidos_Cerrados_vad ) ", cnn, adOpenDynamic, adLockOptimistic
      If Not rs.EOF Then
      ...
      If var_posible_Cerrar = 1 Then
         ...
         With comandoORA
              ...
              'Set parametro = .CreateParameter(, adNumeric, adParamInput, 100, CDbl(Me.txt_embarque))
              '.Parameters.Append parametro
              If Mid(Me.lv_cajas.selectedItem.Text, 1, 3) = "VAD" Then
                Set parametro = .CreateParameter(, adNumeric, adParamInput, 100, CDbl(-1))
              Else
         ...
         If var_posible_cerrado_comparacion = 1 Then
            If Mid(Me.lv_cajas.selectedItem.Text, 1, 3) = "VAD" Then
                var_pedido = Me.lv_cajas.selectedItem.SubItems(1)
            Else
            ...
            If var_pedido >= 10000002 Then
               ...
               Call Command4_Click
            Else
                If IsNumeric(var_pedido) And Mid(Me.lv_cajas.selectedItem.Text, 1, 3) = "VAD" Then
                    If rs.State = 1 Then
                       rs.Close
                    End If
                    Call cerrar_pedido
...
'VAR_ESTATUS = "E"
   ...
   Dim v_pedidonum As Double
   If Mid(Me.lv_cajas.selectedItem.Text, 1, 3) = "VAD" Then
    v_pedidonum = 10000003
   Else
    v_pedidonum = CDbl(Me.lv_cajas.selectedItem.SubItems(1))
   End If
   v_pedido = Me.lv_cajas.selectedItem.SubItems(1)
   If v_pedidonum < 10000002 Then
        rs.Open "ALTER SESSION SET NLS_LANGUAGE = 'AMERICAN'", cnnoracle_4, adOpenDynamic, adLockOptimistic
                  ...
                  While Not rsaux6.EOF
                        ...
                        If rsaux7!ORDER_TYPE_ID = 1002 Or rsaux7!ORDER_TYPE_ID = 1023 Then
                           ...
                           If Not rsaux7.EOF Then
                              ...
                              End If
                              ' se clausura por el problema del 020222
                              'var_correo = ""
                              If VAR_CORREO <> "" Then
...
'-----
   ...
   Else
        If Mid(Me.lv_cajas.selectedItem.Text, 1, 3) = "VAD" Then
            rsaux1.Open "insert into tb_oracle_pedidos_confirmados_vad (pedido, fecha, maquina, error) values ('" + v_pedido + "', getdate(), '" + fun_NombrePc + "',1)", cnn, adOpenDynamic, adLockOptimistic
            rsaux14.Open "UPDATE TB_ORACLE_PEDIDOS_ASIGNADOS_EMBARQUES_VAD SET ESTATUS = 'S' WHERE PEDIDO  = '" + CStr(v_pedido) + "' AND EMBARQUE = " + Me.txt_embarque, cnn, adOpenDynamic, adLockOptimistic
            rsaux14.Open "update tb_oracle_cajas_aduana_VAD set estatus = 'S' where PEDIDO  = '" + CStr(v_pedido) + "'", cnn, adOpenDynamic, adLockOptimistic
            rsaux14.Open "update XXVIA.XXVIA_TB_PEDIDOS_BULTOS_CDI set VCHA_ESTATUS = 'Pedido subido al camion' WHERE NUMB_PEDIDO = '" + CStr(v_pedido) + "'", cnnoracle_4, adOpenDynamic, adLockOptimistic
            rsaux14.Open "update TB_ORACLE_TIEMPO_PEDIDO_ADUANAS_VAD set HORA_FIN = GETDATE() where pedido = '" + CStr(v_pedido) + "'", cnn, adOpenDynamic, adLockOptimistic
...
'------------------
   ...
   Me.Command3.Caption = "DETENER"
   rs.Open "select isnull(sum(PIEZAS),0) PIEZAS from ( SELECT PIEZAS FROM tb_oracle_cajas_aduana WHERE ESTATUS = 'L' AND EMBARQUE = " + Me.txt_embarque + " union all SELECT PIEZAS FROM tb_oracle_cajas_aduana_vad WHERE ESTATUS = 'L' AND EMBARQUE = " + Me.txt_embarque + " ) ped ", cnn, adOpenDynamic, adLockOptimistic
   Me.txt_cantidad = Format(IIf(IsNull(rs(0).Value), 0, rs(0).Value), "###,###,##0.00")
   rs.Close
   var_contador = 1
   rs.Open "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, ISNULL(PAQUETERIA,0) PAQUETERIA from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) " & _
            "union all select AGENTE, nombre_agente, pedido + '-VAD' pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, ISNULL(PAQUETERIA,0) PAQUETERIA from tb_oracle_pedidos_asignados_embarques_vad WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) order by orden_pedido, pedido", cnn, adOpenDynamic, adLockOptimistic
            Text2 = "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, ISNULL(PAQUETERIA,0) PAQUETERIA from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) " & _
            "union all select AGENTE, nombre_agente, pedido + '-VAD' pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, ISNULL(PAQUETERIA,0) PAQUETERIA from tb_oracle_pedidos_asignados_embarques_vad WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) order by orden_pedido, pedido"
   Me.lv_cajas.ListItems.Clear
   Me.lv_cajas_siguientes.ListItems.Clear
   While Not rs.EOF
         If var_contador = 1 Then
            ...
            var_cadena = "select * from tb_oracle_cajas_aduana where embarque = " + Me.txt_embarque + " and pedido = " + CStr(var_pedido) + " and (ISNULL(estatus,'') = '' OR ESTATUS = 'L') union all select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(var_pedido) + "' and (ISNULL(estatus,'I') = 'I' OR ESTATUS = 'L')"
            If IsNumeric(var_pedido) Then
                rsaux10.Open "select * from tb_oracle_cajas_aduana where embarque = " + Me.txt_embarque + " and pedido = " + CStr(var_pedido) + " and (ISNULL(estatus,'') = '' OR ESTATUS = 'L') union all select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(var_pedido) + "' and (ISNULL(estatus,'I') = 'I' OR ESTATUS = 'L')", cnn, adOpenDynamic, adLockOptimistic
            Else
                rsaux10.Open "select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(Replace(var_pedido, "-VAD", "")) + "' and (ISNULL(estatus,'I') = 'I' OR ESTATUS = 'L')", cnn, adOpenDynamic, adLockOptimistic
            End If
            
            While Not rsaux10.EOF
                  rsaux12.Open "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) and pedido = '" + CStr(rs!pedido) + "' union all select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques_vad WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) and pedido = '" + CStr(Replace(rs!pedido, "-VAD", "")) + "' order by orden_pedido, pedido", cnn, adOpenDynamic, adLockOptimistic
                  If Not rsaux12.EOF Then
         ...
         Else
            If IsNumeric(var_pedido) Then
                ...
                rsaux10.Open "select * from tb_oracle_cajas_aduana where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(rs!pedido) + "' and pedido <>'" + CStr(var_pedido) + "' and estatus = '' union all select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(rs!pedido) + "' and pedido <>'" + CStr(var_pedido) + "' and estatus = 'I'", cnn, adOpenDynamic, adLockOptimistic
            Else
                rsaux10.Open "select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(Replace(rs!pedido, "-VAD", "")) + "' and pedido <>'" + CStr(Replace(var_pedido, "-VAD", "")) + "' and estatus = 'I'", cnn, adOpenDynamic, adLockOptimistic
            End If
...
'------------------
   ...
   rs.Close
   var_contador = 1
   rs.Open "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) union all select AGENTE, nombre_agente, pedido + '-VAD' pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques_vad  WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) order by orden_pedido, pedido", cnn, adOpenDynamic, adLockOptimistic
   Me.lv_cajas.ListItems.Clear
   Me.lv_cajas_siguientes.ListItems.Clear
   Text1 = "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) union all select AGENTE, nombre_agente, pedido + '-VAD' pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques_vad  WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) order by orden_pedido, pedido"
   While Not rs.EOF
         If var_contador = 1 Then
            var_pedido = rs!pedido
            If IsNumeric(var_pedido) Then
            ...
            While Not rsaux10.EOF
                ...
                If IsNumeric(rs!pedido) Then
                    rsaux12.Open "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'') = '')) and pedido = " + CStr(rs!pedido) + " union all select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques_vad WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) and pedido = '" + CStr(rs!pedido) + "' order by orden_pedido, pedido", cnn, adOpenDynamic, adLockOptimistic
                Else
                    rsaux12.Open "select AGENTE, nombre_agente, pedido, cliente, orden_pedido, ESTATUS, estatus_pedido, isnull(paqueteria,0) paqueteria from tb_oracle_pedidos_asignados_embarques_vad WHERE EMBARQUE = " + Me.txt_embarque + " AND ((ISNULL(ESTATUS,'I') = 'I')) and pedido = '" + CStr(Replace(rs!pedido, "-VAD", "")) + "' order by orden_pedido, pedido", cnn, adOpenDynamic, adLockOptimistic
                End If
            ...
            rsaux10.Close
         Else
            If IsNumeric(rs!pedido) And IsNumeric(var_pedido) Then
                rsaux10.Open "select * from tb_oracle_cajas_aduana where embarque = " + Me.txt_embarque + " and pedido = " + CStr(rs!pedido) + " and pedido <>" + CStr(var_pedido) + " and estatus = '' union all select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(rs!pedido) + "' and pedido <> '" + CStr(var_pedido) + "' and estatus = 'I'", cnn, adOpenDynamic, adLockOptimistic
            Else
                rsaux10.Open "select * from tb_oracle_cajas_aduana_vad where embarque = " + Me.txt_embarque + " and pedido = '" + CStr(Replace(rs!pedido, "-VAD", "")) + "' and pedido <> '" + CStr(var_pedido) + "' and estatus = 'I'", cnn, adOpenDynamic, adLockOptimistic
            End If
...
Private Sub txt_codigo_KeyPress(KeyAscii As Integer)
   Dim var_pedido_tiempo As String
   Dim clnt As New SoapClient30
      ...
      If Trim(Me.txt_codigo) <> "" Then
         For var_j = 1 To lv_cajas.ListItems.Count
             ...
             If Me.txt_codigo = Me.lv_cajas.selectedItem Then
                var_pedido_tiempo = Me.lv_cajas.selectedItem.SubItems(1)
                var_encontro = 1
                If Me.lv_cajas.selectedItem.SubItems(5) = "L" Then
                   ...
                   Call ilumina_grid
                   rsaux11.Open "SELECT * FROM TB_ORACLE_BITACORA_CAJAS_ADUANA WHERE CAJA = '" + Me.lv_cajas.selectedItem + "'", cnn, adOpenDynamic, adLockOptimistic
                   If rsaux11.EOF Then
                      rsaux10.Open "INSERT INTO TB_ORACLE_BITACORA_CAJAS_ADUANA (EMBARQUE, PEDIDO, CAJA, MAQUINA, DVR, PUERTO) VALUES ('" + Me.txt_embarque + "','" + Me.lv_cajas.selectedItem.SubItems(1) + "','" + Me.lv_cajas.selectedItem + "','" + fun_NombrePc + "','" + var_dvr_texto + "','" + var_puerto_texto + "')", cnn, adOpenDynamic, adLockOptimistic
                   End If
                   rsaux11.Close
                ...
                Else
                   ...
                   blnBandera = False
                   If IsNumeric(var_pedido_tiempo) Then
                        If var_pedido_tiempo >= 10000002 Then
                            blnBandera = True
                        End If
                   End If
                   
                   If blnBandera = True And Mid(Me.txt_codigo.Text, 1, 3) <> "VAD" Then
                    
                      rs.Open "select * from TB_ORACLE_CAJAS_ADUANA where pedido = " + CStr(var_pedido_tiempo) + " union all select * from TB_ORACLE_CAJAS_ADUANA_vad where pedido = '" + CStr(var_pedido_tiempo) + "'", cnn, adOpenDynamic, adLockOptimistic
                      ...
                      rs.Close
                   Else
                        If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                            'var_referencia_embarque = Mid(Me.txt_codigo, 4, Len(txt_codigo.Text) - 6)
                            var_referencia_caja = Mid(Me.txt_codigo, Len(txt_codigo.Text) - 2)
                   ...
                   If IsNumeric(var_referencia_embarque) Then
                       If IsNumeric(var_referencia_caja) Then
                          If CDbl(Me.txt_embarque) = CDbl(var_referencia_embarque) Then
                             ...
                             var_posible_peso = 1
                             If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                strconsulta = "select bul.NUMB_PEDIDO source_header_number, bul.NUMB_CAJA_ID inte_paq_caja, bul.VCHA_TIPO_CAJA tipo_caja, sum( bul.NUMB_CANTIDAD * nvl(sib.UNIT_WEIGHT,0.55)) peso, sum(bul.NUMB_CANTIDAD) cantidad from XXVIA.XXVIA_TB_PEDIDOS_BULTOS_CDI bul, INV.MTL_SYSTEM_ITEMS_B sib where bul.NUMB_EMBARQUE = ? and bul.NUMB_PEDIDO_ID || bul.NUMB_CAJA_ID = ? and sib.ORGANIZATION_ID = 84 and sib.SEGMENT1 = bul.VCHA_CODIGO group by bul.NUMB_PEDIDO, bul.NUMB_CAJA_ID, bul.VCHA_TIPO_CAJA"
                             Else
                             ...
                             With comandoORA
                                  ...
                                  .Parameters.Append parametro
                                  If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                    Set parametro = .CreateParameter(, adVarChar, adParamInput, 200, Replace(txt_codigo.Text, "VAD", ""))
                                  Else
                             ...
                             rs.Open "select embarque, cast(pedido as varchar(50)) pedido , numero_caja, caja, agente, cliente, establecimiento, piezas, estatus, tipo_empaque,caja_pedido,sello, lote,guia,embarque_anterior, pedido_anterior,caja_anterior,marca_cambio_embarque,embarque_actual marca_cambio_embarque_2 from tb_oracle_cajas_aduana where embarque  = " + CStr(var_referencia_embarque) + " and CAJA = '" + CStr(Me.txt_codigo) + "' union all select embarque, pedido, numero_caja, caja, agente, cliente, establecimiento, piezas, estatus, tipo_empaque,caja_pedido,sello, lote,guia,embarque_anterior, pedido_anterior,caja_anterior,marca_cambio_embarque,embarque_actual marca_cambio_embarque_2 from tb_oracle_cajas_aduana_vad where embarque  = " + CStr(var_referencia_embarque) + " and CAJA = '" + CStr(Me.txt_codigo) + "'", cnn, adOpenDynamic, adLockOptimistic
                             If Not rs.EOF Then
                                var_estatus_caja = IIf(IsNull(rs!estatus), "", rs!estatus)
                                If var_estatus_caja = "" Or var_estatus_caja = "I" Then
                                   var_embarque_auditar = CDbl(Me.txt_embarque)
                                   var_caja_auditar = CDbl(var_referencia_caja)
                                   ...
                                   If Me.lv_cajas.selectedItem.SubItems(7) = "COSTAL GRANDE" Or Me.lv_cajas.selectedItem.SubItems(7) = "COSTAL CHICO" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA  SOBRE-CAJA" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA CHICA" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA CORTINERO" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA EXTRAGRANDE" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA GRANDE" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA MEDIANA" Or Me.lv_cajas.selectedItem.SubItems(7) = "CAJA MINI/CATALOGO" Or Me.lv_cajas.selectedItem.SubItems(7) = "COSTAL CHICO" Or Me.lv_cajas.selectedItem.SubItems(7) = "COSTAL GRANDE" Or Me.lv_cajas.selectedItem.SubItems(7) = "EMPLAYE CORTINEROS" Or Me.lv_cajas.selectedItem.SubItems(7) = "OTROS" Or Me.lv_cajas.selectedItem.SubItems(7) = "OTROS MUEBLES" Or Me.lv_cajas.selectedItem.SubItems(7) = "PAQUETE BOLSA" Or Me.lv_cajas.selectedItem.SubItems(7) = "PAQUETE PUBLICIDAD" Then
                                    If Mid(Me.txt_codigo.Text, 1, 3) = "VAD" Then
                                         var_auditar = 0
                                    Else
                                   ...
                                   If var_auditar = 1 Then
                                        If Mid(Me.txt_codigo.Text, 1, 3) = "VAD" Then
                                            Dim v_s As String
                                            v_s = "SELECT NVL(AUDITADA,0) AS AUDITADA from XXVIA.XXVIA_TB_PEDIDOS_BULTOS_CDI bul where bul.NUMB_EMBARQUE = " + CStr(var_referencia_embarque) + " and bul.NUMB_PEDIDO_ID = " + Mid(Me.txt_codigo.Text, 4, Len(Me.txt_codigo.Text) - 6) + " and bul.NUMB_CAJA_ID = " + CStr(CInt(Mid(Me.txt_codigo.Text, Len(Me.txt_codigo.Text) - 2))) + " "
                                            rsaux.Open "SELECT NVL(AUDITADA,0) AS AUDITADA from XXVIA.XXVIA_TB_PEDIDOS_BULTOS_CDI bul where bul.NUMB_EMBARQUE = " + CStr(var_referencia_embarque) + " and bul.NUMB_PEDIDO_ID = " + Mid(Me.txt_codigo.Text, 4, Len(Me.txt_codigo.Text) - 6) + " and bul.NUMB_CAJA_ID = " + CStr(CInt(Mid(Me.txt_codigo.Text, Len(Me.txt_codigo.Text) - 2))) + " ", cnnoracle_4, adOpenDynamic, adLockOptimistic
                                        Else
                                   ...
                                   If var_auditar = 1 Then
                                      ...
                                      If Me.lbl_paqueteria <> "" Then
                                         ...
                                         If rsaux13.EOF Then
                                            ...
                                            strconsulta = "SELECT * FROM XXVIA.XXVIA_TB_PAQUETERIAS_GUIAS where vcha_guia=REPLACE(REPLACE(?,'!%1!%1',''),'!%1!%1','') or numb_rastreo = REPLACE(REPLACE(?,'!%1!%1',''),'!%1!%1','')"
                                            With comandoORA
                                                 'MsgBox cnnoracle_4.ConnectionString
                                                 .ActiveConnection = cnnoracle_4
                                                 .CommandType = adCmdText
                                      ...
                                      If var_posible_guia = 1 Then
                                         rsaux1.Open "update tb_oracle_cajas_aduana set estatus = 'L', guia = '" + var_guia_aduana + "', fecha_aduana = getdate() where embarque = " + CStr(Me.txt_embarque) + " and numero_caja = " + CStr(var_caja_auditar), cnn, adOpenDynamic, adLockOptimistic
                                         ...
                                         Set parametro = Nothing
                                        If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                            var_caja_auditar = CDbl(Replace(txt_codigo.Text, "VAD", ""))
                                        End If
                                         frmoracle_audita_caja.Show 1
                                         rsaux.Open "SELECT * FROM XXVIA_TB_cAJAS_AUDITADAS WHERE EMBARQUE = " + CStr(var_embarque_auditar) + " AND CAJA = " + CStr(var_caja_auditar) + " AND CANTIDAD_ORIGINAL <> CANTIDAD_AUDITADA", cnnoracle_4, adOpenDynamic, adLockOptimistic
                                         If rsaux.EOF Then
                                            ...
                                            rsaux1.Open "update xxvia_tb_salidas_cajas set sello = '" + var_sello_caja + "', AUDITADA = 1, OBSERVACIONES_AUDITORIA = '" + var_observaciones_auditoria + "' where inte_emb_embarque = " + CStr(Me.txt_embarque) + " and inte_paq_caja = " + CStr(var_caja_auditar), cnnoracle_4, adOpenDynamic, adLockOptimistic
                                            rsaux1.Open "update tb_oracle_cajas_aduana set estatus = 'L', fecha_aduana = getdate() where embarque = " + CStr(Me.txt_embarque) + " and numero_caja = " + CStr(var_caja_auditar), cnn, adOpenDynamic, adLockOptimistic
                                            
                                            rsaux11.Open "SELECT * FROM TB_ORACLE_BITACORA_CAJAS_ADUANA WHERE CAJA = '" + Me.txt_codigo + "'", cnn, adOpenDynamic, adLockOptimistic
                                            If rsaux11.EOF Then
                                               rsaux10.Open "INSERT INTO TB_ORACLE_BITACORA_CAJAS_ADUANA (EMBARQUE, PEDIDO, CAJA, MAQUINA, DVR, PUERTO) VALUES ('" + Me.txt_embarque + "','" + Me.lv_cajas.selectedItem.SubItems(1) + "','" + Me.lv_cajas.selectedItem + "','" + fun_NombrePc + "','" + var_dvr_texto + "','" + var_puerto_texto + "')", cnn, adOpenDynamic, adLockOptimistic
                                            End If
                                            rsaux11.Close
                                   ...
                                   Else
                                      ...
                                      If Me.lbl_paqueteria <> "" Then
                                         ...
                                         Else
                                            ...
                                            If Not rsaux8.EOF Then
                                               ...
                                               While Not rsaux8.EOF
                                                    If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                                        var_caja_aduana = "VAD" + IIf(IsNull(rsaux8!vcha_caja_id), "", rsaux8!vcha_caja_id)
                                                    Else
                                                        var_caja_aduana = "C" + IIf(IsNull(rsaux8!vcha_caja_id), "", rsaux8!vcha_caja_id)
                                                    End If
                                      ...
                                      If var_posible_guia = 1 Then
                                        If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                             rsaux1.Open "update tb_oracle_cajas_aduana_vad set estatus = 'L', guia = '" + var_guia_aduana + "', fecha_aduana = getdate() where embarque = " + CStr(Me.txt_embarque) + " and caja = '" + CStr(txt_codigo.Text) + "'", cnn, adOpenDynamic, adLockOptimistic
                                             strconsulta = "update XXVIA.XXVIA_TB_PEDIDOS_BULTOS_CDI set VCHA_GUIA_AUDITADA = ?, VCHA_TRANSPORTE = ? where NUMB_EMBARQUE = ? and NUMB_PEDIDO_ID  = ?  AND NUMB_CAJA_ID = ? "
                                        Else
                                             rsaux1.Open "update tb_oracle_cajas_aduana set estatus = 'L', guia = '" + var_guia_aduana + "', fecha_aduana = getdate() where embarque = " + CStr(Me.txt_embarque) + " and numero_caja = " + CStr(var_caja_auditar), cnn, adOpenDynamic, adLockOptimistic
                                             strconsulta = "update xxvia_Tb_salidas_cajas set guia = ?, transporte = ? where inte_emb_embarque = ? and inte_paq_caja = ?"
                                        End If
                                         ...
                                         With comandoORA
                                              ...
                                              .Parameters.Append parametro
                                              If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                                Set parametro = .CreateParameter(, adNumeric, adParamInput, 200, Mid(txt_codigo.Text, 4, Len(txt_codigo.Text) - 6))
                                                .Parameters.Append parametro
                                                Set parametro = .CreateParameter(, adNumeric, adParamInput, 200, CInt(Mid(txt_codigo.Text, Len(txt_codigo.Text) - 2)))
                                              Else
                                                Set parametro = .CreateParameter(, adNumeric, adParamInput, 8, CDbl(var_caja_auditar))
                                              End If
                                              .Parameters.Append parametro
                                         ...
                                         'CREAR OTRA TABLA GEMELA PARA LOS MEDIR EL TIEMPO DE LOS PEDIDOS
                                         
                                       If Mid(txt_codigo.Text, 1, 3) = "VAD" Then
                                           rsaux1.Open "select * from tb_oracle_tiempo_pedido_aduanas_vad where pedido = '" + CStr(var_pedido_tiempo) + "'", cnn, adOpenDynamic, adLockOptimistic
                                           If rsaux1.EOF Then
                                              rsaux10.Open "insert into tb_oracle_tiempo_pedido_aduanas_vad (pedido, hora_inicio) values (" + CStr(var_pedido_tiempo) + ",getdate())", cnn, adOpenDynamic, adLockOptimistic
                                           End If
                                       Else
                                           rsaux1.Open "select * from tb_oracle_tiempo_pedido_aduanas where pedido = " + CStr(var_pedido_tiempo), cnn, adOpenDynamic, adLockOptimistic
                                           If rsaux1.EOF Then
