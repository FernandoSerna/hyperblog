...
Begin VB.Form frmdescuentos_volumen_cliente 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Descuentos por volumen a cliente"
   ClientHeight    =   6720
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9255
   ...
   Begin VB.Frame frm_periodo 
      ...
      Begin VB.CommandButton cmd_cambiar_periodo 
         ...
         Height          =   315
         Left            =   4350
         Picture         =   "frmdescuentos_volumen_cliente.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   31
      ...
      Begin VB.ListBox lst_años 
         ...
         EndProperty
         Height          =   300
         ItemData        =   "frmdescuentos_volumen_cliente.frx":014A
         Left            =   3405
         List            =   "frmdescuentos_volumen_cliente.frx":018D
         TabIndex        =   28
         Top             =   255
         Width           =   900
      End
      Begin VB.ComboBox cmb_meses 
         Height          =   315
         ItemData        =   "frmdescuentos_volumen_cliente.frx":020F
         Left            =   630
         List            =   "frmdescuentos_volumen_cliente.frx":0237
         TabIndex        =   27
         Top             =   240
         Width           =   2280
      End
   ...
   Begin VB.CommandButton cmd_aplicar 
      ...
      Height          =   315
      Left            =   465
      Picture         =   "frmdescuentos_volumen_cliente.frx":02A0
      Style           =   1  'Graphical
      TabIndex        =   23
      ToolTipText     =   "Aplicar Pagos Alt + A"
      Top             =   0
   ...
   Begin VB.CommandButton Command1 
      ...
      Height          =   315
      Left            =   795
      Picture         =   "frmdescuentos_volumen_cliente.frx":03EA
      Style           =   1  'Graphical
      TabIndex        =   22
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CommandButton cmd_periodo 
         ...
         Height          =   315
         Left            =   8475
         Picture         =   "frmdescuentos_volumen_cliente.frx":057C
         Style           =   1  'Graphical
         TabIndex        =   25
   ...
   Begin VB.CommandButton cmd_salir 
      ...
      Height          =   315
      Left            =   8715
      Picture         =   "frmdescuentos_volumen_cliente.frx":06B2
      Style           =   1  'Graphical
      TabIndex        =   17
      ToolTipText     =   "Salir"
      Top             =   0
   ...
   Begin VB.CommandButton cmd_buscar 
      ...
      Height          =   315
      Left            =   135
      Picture         =   "frmdescuentos_volumen_cliente.frx":0CEC
      Style           =   1  'Graphical
      TabIndex        =   16
   ...
   Begin VB.CommandButton cmd_imprimir 
      ...
      Height          =   315
      Left            =   1125
      Picture         =   "frmdescuentos_volumen_cliente.frx":0DEE
      Style           =   1  'Graphical
      TabIndex        =   15
      ToolTipText     =   "Imprimir Alt + I"
      Top             =   0
   ...
   End
End
Attribute VB_Name = "frmdescuentos_volumen_cliente"
...
Private Sub cmd_aplicar_Click()
   ...
   If si = 6 Then
      rs.Open "update tb_descuentos_volumen_asignado set floa_dvo_descuento_corregido = " + txt_descuento_aplicar + ", vcha_dvo_causa_correccion = '" + txt_causa + "' where vcha_cli_clave_id = '" + txt_grupo + "' and DTIM_DVO_PERIODO_INICIO  = '" + CStr(fecha_inicio) + "' and DTIM_DVO_PERIODO_FIN = '" + CStr(fecha_fin + 1) + "'", cnn, adOpenDynamic
...
Dim si As Integer
   ...
   If si = 6 Then
      ...
      If si = 6 Then
         ...
         While Not rs.EOF
            rsaux2.Open "update tb_gruposactuales set floa_gac_descuento_1 = " + CStr(rs!floa_dvo_descuento_corregido) + " where vcha_gre_grupo_real_id = '" + rs!vcha_gre_grupo_real_id + "'", cnn, adOpenDynamic, adLockOptimistic
            rs.MoveNext
         Wend
         rs.Close
...
Private Sub Form_Unload(Cancel As Integer)
   ...
   Call activa_forma(var_activa_forma_descuentos_volumen_cliente)
End Sub
...
Private Sub txt_agente_LostFocus()
   If Trim(txt_agente) <> "" Then
      ...
      If Not rs.EOF Then
         ...
         rsaux2.Open "select distinct vcha_cli_clave_id,vcha_cli_nombre, floa_dvo_importe_cliente,floa_dvo_descuento, floa_dvo_descuento_corregido, vcha_dvo_causa_correccion from vw_descuentos_volumen where vcha_age_agente_id = '" + txt_agente + "' and DTIM_DVO_PERIODO_INICIO  = '" + CStr(fecha_inicio) + "' and DTIM_DVO_PERIODO_FIN = '" + CStr(fecha_fin + 1) + "'", cnn, adOpenDynamic
         If Not rsaux2.EOF Then
            Dim list_item As ListItem
            contador = 0
            lv_grupos.ListItems.Clear
            While Not rsaux2.EOF
               Set list_item = lv_grupos.ListItems.Add(, , rsaux2!vcha_cli_clave_id)
               list_item.SubItems(1) = IIf(IsNull(rsaux2!VCHA_CLI_NOMBRE), "", rsaux2!VCHA_CLI_NOMBRE)
               list_item.SubItems(2) = Format(IIf(IsNull(rsaux2!floa_dvo_importe_cliente), 0, rsaux2!floa_dvo_importe_cliente), "###,###,##0.00")
...
Private Sub txt_cobranza_KeyPress(KeyAscii As Integer)
   If KeyAscii = 39 Or KeyAscii = 61 Then
      KeyAscii = 0
   End If
