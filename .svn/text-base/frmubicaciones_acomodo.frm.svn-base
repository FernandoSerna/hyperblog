...
Begin VB.Form frmubicaciones_acomodo 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Ubicar entradas"
   ClientHeight    =   7290
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   8115
   ...
   End
End
Attribute VB_Name = "frmubicaciones_acomodo"
...
Private Sub lv_detalle_movimiento_KeyDown(KeyCode As Integer, Shift As Integer)
   If KeyCode = 114 Then
      ...
      rs.Open "SELECT * FROM TB_UBICACIONES_ENTRADAS WHERE VCHA_EMP_EMPRESA_ID = '" + var_empresa + "' AND VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "' AND VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "' AND INTE_ENT_NUMERO = " + Me.txt_numero + " AND VCHA_ART_ARTICULO_ID = '" + Me.lv_detalle_movimiento.selectedItem + "'", cnn, adOpenDynamic, adLockOptimistic
      While Not rs.EOF
            Set list_item = lv_lista.ListItems.Add(, , IIf(IsNull(rs!VCHA_UBI_UBICACION), "", rs!VCHA_UBI_UBICACION))
            list_item.SubItems(1) = IIf(IsNull(rs!FLOA_ENT_CANTIDAD), "", rs!FLOA_ENT_CANTIDAD)
            rs.MoveNext:
      Wend
      rs.Close
   ...
   If KeyAscii = 13 Then
      If lv_lista.ListItems.Count > 0 Then
         ...
         If var_si = 6 Then
            ...
            rs.Open "DELETE FROM TB_UBICACIONES_ENTRADAS WHERE VCHA_EMP_EMPRESA_ID = '" + var_empresa + "' AND VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "' AND VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "' AND INTE_ENT_NUMERO = " + Me.txt_numero + " AND VCHA_aRT_aRTICULO_ID = '" + Trim(Me.lv_detalle_movimiento.selectedItem) + "' AND VCHA_UBI_UBICACION = '" + Trim(Me.lv_lista.selectedItem) + "'", cnn, adOpenDynamic, adLockOptimistic
      ...
      If Me.txt_movimiento <> "" Then
         If IsNumeric(Me.txt_numero) Then
            If IsNumeric(Me.txt_cantidad) Then
               rs.Open "SELECT * FROM TB_ENTRADAS WHERE VCHA_EMP_EMPRESA_ID = '" + var_empresa + "' AND VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "' AND VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "' AND INTE_ENT_NUMERO = " + Me.txt_numero + " AND VCHA_aRT_ARTICULO_ID = '" + Me.txt_codigo + "'", cnn, adOpenDynamic, adLockOptimistic
               If Not rs.EOF Then
                  var_cantidad = IIf(IsNull(rs!FLOA_ENT_CANTIDAD), 0, rs!FLOA_ENT_CANTIDAD)
                  VAR_CANTIDAD_UBICADA = IIf(IsNull(rs!FLOA_ENT_CANTIDAD_UBICADA), 0, rs!FLOA_ENT_CANTIDAD_UBICADA)
                  ...
                  If var_cantidad >= VAR_CANTIDAD_UBICADA + var_cantidad_leida Then
                     ...
                     rsaux.Open "SELECT * FROM TB_UBICACIONES_ENTRADAS WHERE VCHA_EMP_EMPRESA_ID = '" + var_empresa + "' AND VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "' AND VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "' AND INTE_ENT_NUMERO = " + Me.txt_numero + " AND VCHA_aRT_ARTICULO_ID = '" + Me.txt_codigo + "' AND VCHA_UBI_UBICACION = '" + Me.txt_ubicacion + "'", cnn, adOpenDynamic, adLockOptimistic
                     If Not rsaux.EOF Then
                        rsaux1.Open "UPDATE TB_UBICACIONES_ENTRADAS SET FLOA_ENT_CANTIDAD = ISNULL(FLOA_ENT_CANTIDAD,0) + " + Me.txt_cantidad + " WHERE VCHA_EMP_EMPRESA_ID = '" + var_empresa + "' AND VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "' AND VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "' AND INTE_ENT_NUMERO = " + Me.txt_numero + " AND VCHA_aRT_ARTICULO_ID = '" + Me.txt_codigo + "' AND VCHA_UBI_UBICACION = '" + Me.txt_ubicacion + "'", cnn, adOpenDynamic, adLockOptimistic
                     Else
                        rsaux1.Open "INSERT INTO TB_UBICACIONES_ENTRADAS (VCHA_EMP_EMPRESA_ID, VCHA_UOR_UNIDAD_ID, VCHA_MOV_MOVIMIENTO_ID, INTE_ENT_NUMERO, VCHA_ART_ARTICULO_ID, VCHA_UBI_UBICACION, FLOA_ENT_CANTIDAD) VALUES ('" + var_empresa + "', '" + var_unidad_organizacional + "','" + Me.txt_movimiento + "', " + Me.txt_numero + ",'" + Me.txt_codigo + "','" + Me.txt_ubicacion + "'," + Me.txt_cantidad + ")", cnn, adOpenDynamic, adLockOptimistic
                     End If
                     rsaux.Close
                     Me.txt_ubicacion.SetFocus
                  Else
...
Private Sub txt_numero_LostFocus()
   If Me.txt_movimiento <> "" Then
      If IsNumeric(Me.txt_numero) Then
         ...
         Me.txt_codigo = ""
         Me.txt_cantidad = ""
         var_cadena = "SELECT dbo.TB_ENTRADAS.VCHA_EMP_EMPRESA_ID, dbo.TB_ENTRADAS.VCHA_UOR_UNIDAD_ID, dbo.TB_ENTRADAS.VCHA_MOV_MOVIMIENTO_ID, dbo.TB_ENTRADAS.DTIM_ENT_FECHA, dbo.TB_ENTRADAS.INTE_ENT_NUMERO, dbo.TB_ENTRADAS.VCHA_ART_ARTICULO_ID, dbo.TB_ARTICULOS.VCHA_ART_NOMBRE_ESPAÑOL, dbo.TB_ENTRADAS.FLOA_ENT_CANTIDAD, ISNULL(dbo.TB_ENTRADAS.FLOA_ENT_CANTIDAD_UBICADA, 0) As FLOA_ENT_CANTIDAD_UBICADA FROM dbo.TB_ENTRADAS INNER JOIN dbo.TB_ARTICULOS ON dbo.TB_ENTRADAS.VCHA_ART_ARTICULO_ID = dbo.TB_ARTICULOS.VCHA_ART_ARTICULO_ID WHERE (dbo.TB_ENTRADAS.INTE_ENT_NUMERO = " + Me.txt_numero + ") AND (dbo.TB_ENTRADAS.VCHA_EMP_EMPRESA_ID = '" + var_empresa + "') AND (dbo.TB_ENTRADAS.VCHA_UOR_UNIDAD_ID = '" + var_unidad_organizacional + "') AND (dbo.TB_ENTRADAS.VCHA_MOV_MOVIMIENTO_ID = '" + Me.txt_movimiento + "')"
         rs.Open var_cadena, cnn, adOpenDynamic, adLockOptimistic
         If Not rs.EOF Then
            Me.txt_fecha = rs!dtim_ent_Fecha
            While Not rs.EOF
                  ...
                  list_item.SubItems(2) = IIf(IsNull(rs!FLOA_ENT_CANTIDAD), 0, rs!FLOA_ENT_CANTIDAD)
                  list_item.SubItems(3) = IIf(IsNull(rs!FLOA_ENT_CANTIDAD_UBICADA), 0, rs!FLOA_ENT_CANTIDAD_UBICADA)
                  rs.MoveNext:
            Wend
         Else
